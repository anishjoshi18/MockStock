...
<head>
    ...
    function validateForm()
		...
		if(typeof(EventSource) !== "undefined") {
			var source = new EventSource("news_apply.php");
                                ...
                                if($run = mysqli_query($conn, $query))
                                {
                                    if(mysqli_num_rows($run) >= 1)
                                    {
                                        ...
                                        while($array = mysqli_fetch_assoc($run))
                                        {
                                            ...
                                            echo "<td><span class='$class'><div id=\"companyprice\">$company_price $change</div> </span></td>
                                            <td>$high</td>
                                            <td>$low</td>";
                                            echo "</tr>";
                                        }
...
<!--
	...
	<script type="text/javascript">
		function updatePrice() {
			var url = "getpricevalue.php?id=123&time=" + (new Date()).getTime();
