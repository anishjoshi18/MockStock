...
if(isset($_GET['id']) && isset($_POST['heading']) && isset($_POST['n_description']))
{

    $id = $_GET['id'];
    $heading = $_POST['heading'];
    $n_description = nl2br($_POST['n_description']);
    
    $query = "UPDATE news SET heading = '$heading', description = '$n_description' WHERE id = $id";
    
    
    $news = new News($id);
    if(mysqli_query($conn, $query))
    {
        echo "<div id='note'>Details updated for $heading. <a id='close' class='pull-right'>[Close]&nbsp;</a></div>";
    }
    else
        echo "Gandlo";
}
...
if(isset($_POST['news_name']) && isset($_POST['news_description']))
{
    ...
    $query_resgister = "INSERT INTO news(heading, description) VALUES('$name', '$description')";
		...
		if(mysqli_query($conn, $query_resgister))
		{
			...
			echo "<div id='note'>Error Registering...<a id='close' class='pull-right'>[Close]&nbsp;</a></div>";
}
